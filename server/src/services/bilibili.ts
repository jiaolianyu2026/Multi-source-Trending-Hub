 ...
 *
 * 关键字段说明：
 * - data.trending.list: 热搜列表数组
 *   - show_name: 热搜标题（显示用）
 *   - keyword: 搜索关键词（用于构建URL）
 *   - heat_score: 热度数值
 */
interface BilibiliApiResponse {
  data?: {
    trending?: {
      list?: Array<{
        show_name?: string;    // 热搜标题
        keyword?: string;      // 搜索关键词
        heat_score?: number;   // 热度数值
      }>;
    };
  };
}
...
export async function fetchBilibiliHot(): Promise<BilibiliHotItem[]> {
  ...
  try {
    ...
    const result = await response.json() as BilibiliApiResponse;
    ...
    const items: BilibiliHotItem[] = result.data.trending.list
      .slice(0, 10) // 只取前 10 条
      .map((item, index) => {
        const rank = index + 1;
 ...
 * 测试函数（开发时使用）
 */
export async function testFetchBilibili(): Promise<void> {
  try {
