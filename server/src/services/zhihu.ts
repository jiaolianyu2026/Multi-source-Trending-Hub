...
/**
 * 知乎热榜条目
 * 符合 TECH_DESIGN.md 中的 HotSearchItem 格式
 */
...
interface ZhihuApiResponse {
  data?: Array<{
    ...
    detail_text?: string;          // 热度文本（如 "1234 万热度"）
  }>;
}
  ...
  try {
    ...
    const items: ZhihuHotItem[] = result.data
      ...
      .map((item, index) => {
        ...
        const extra: { answerCount?: number; viewCount?: number } = {};
 ...
 * 测试函数（开发时使用）
 */
export async function testFetchZhihu(): Promise<void> {
  try {
