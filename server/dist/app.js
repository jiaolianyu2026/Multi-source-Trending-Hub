...
const weibo_1 = require("./services/weibo");
const zhihu_1 = require("./services/zhihu");
const bilibili_1 = require("./services/bilibili");
// 调试：检查环境变量是否加载
...
async function fetchZhihuData(skipCache = false) {
    ...
    try {
        ...
        const items = await (0, zhihu_1.fetchZhihuHot)();
        const data = {
            source: 'zhihu',
            sourceName: '知乎',
            listName: '热榜',
        ...
        (0, cache_1.setCache)(cacheKey, data);
...
async function fetchBilibiliData(skipCache = false) {
    ...
    try {
        ...
        const items = await (0, bilibili_1.fetchBilibiliHot)();
        const data = {
            source: 'bilibili',
            sourceName: 'B站',
            listName: '热搜榜',
...
async function fetchWeiboData(skipCache = false) {
    ...
    try {
        ...
        const items = await (0, weibo_1.fetchWeiboHot)();
        const data = {
            source: 'weibo',
            sourceName: '微博',
            listName: '热搜榜',
 ...
 * 处理微博热搜请求（真实数据 + 缓存）
 */
async function handleWeiboRequest(req, res) {
    const { refresh } = req.query;
    const cacheKey = 'hot:weibo';
    const isRefresh = refresh === '1';
    // 检查是否强制刷新
 ...
 */
async function handleZhihuRequest(req, res) {
    const { refresh } = req.query;
    const cacheKey = 'hot:zhihu';
    const isRefresh = refresh === '1';
    // 检查是否强制刷新
 ...
 * 返回格式符合 TECH_DESIGN.md
 */
async function handleBilibiliRequest(req, res) {
    const { refresh } = req.query;
    const cacheKey = 'hot:bilibili';
    const isRefresh = refresh === '1';
    // 检查是否强制刷新
